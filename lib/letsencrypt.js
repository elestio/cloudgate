...
async function GenerateCert(isProd, domains, maintainerEmail, certPath, rootFolder, LEAccountPath){
    ...
    var pkPath = path.join(certPath, "privkey.pem");
    var cPath = path.join(certPath, "cert.pem");
    //console.log("checking for certs in: " + metaPath);
    if (fs.existsSync(metaPath) && fs.existsSync(pkPath) && fs.existsSync(cPath)){
        //certs already exist
        ...
        var nbDaysLeft = GetDaysLeftBeforeExpiration(cPath);
    ...
    var packageAgent = 'cloudgate' + '/' + require('../package.json').version;
