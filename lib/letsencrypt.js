...
module.exports.GenerateCert = async (isProd, domains, maintainerEmail, certPath, rootFolder) => {

    return new Promise(resolve => {
        resolve(GenerateCert(isProd, domains, maintainerEmail, certPath, rootFolder));
    });
    //return await GenerateCert(isProd, domains, maintainerEmail, certPath, rootFolder);
}

async function GenerateCert(isProd, domains, maintainerEmail, certPath, rootFolder){

    var metaPath = resolve(certPath + "meta.json");
    if (fs.existsSync(metaPath)){
        //certs already exist
        console.log("Certificate already exist, no need to generate it!");

        //TODO: check if expired or must be renewed
        var result = {
    ...
    var packageAgent = 'cloudgate' + '/' + "1.0.0";
    ...
    var accountPath = resolve(certPath + "letsencrypt") + "/";
    ...
    var accountKey = null;
    var accountKeyPath = accountPath + "account.key";
    ...
    function http01(){
        return {
            ...
            remove: async function(args) {
                ...
                if ( args.challenge.status == 'valid')
                {
                    var date = new Date();
