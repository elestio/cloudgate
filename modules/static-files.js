      ...
      try {
        ...
        try {
          ...
          if (
            ...
            typeof ifmodifiedsince != "undefined" &&
            ifmodifiedsince != ""
          ) {
        ...
        try {
          ...
          if (
            ...
            if (targetMime != null) {
              if (
                targetMime.indexOf("text") > -1 ||
                targetMime.indexOf("json") > -1
              ) {
        ...
        if (appConfig.AWS != null && appConfig.TypeFS == "S3") {
          ...
          try {
            ...
            if (ex.code == "NotFound") {
              ...
              result.content =
                ex.message +
                "<br/>Please check your AWS credentials / region / bucket in appConfig.json";
          ...
          s3.getObject(getParams, function (err, data) {
            if (err) {
              ...
              if (
                ...
                finalPath.endsWith(".xml") ||
                finalPath.endsWith(".txt")
              ) {
                processingNeeded = 1;
              }
              if (processingNeeded) {
        ...
        try {
          ...
          if (fileExist) {
            ...
            result.headers["expires"] = new Date(
              Date.now() + maxAge * 1000
            ).toUTCString();
            result.headers["last-modified"] = new Date(
              Date.now()
            ).toUTCString();
